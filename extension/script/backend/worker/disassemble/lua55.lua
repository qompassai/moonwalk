-- Lua 5.5 opcodes and disassembly

-- opcode names (order from lopcodes.h ORDER OP, 86 total)
local OPCODES = {
    'MOVE',
    'LOADI',
    'LOADF',
...
local OPMODES = {
    ...
    { 0, 0, 0, 0, 1, 'ivABC' }, -- NEWTABLE  (ivABC in 5.5)
    { 0, 0, 0, 0, 1, 'iABC' }, -- SELF
    { 0, 0, 0, 0, 1, 'iABC' }, -- ADDI
    { 0, 0, 0, 0, 1, 'iABC' }, -- ADDK
    { 0, 0, 0, 0, 1, 'iABC' }, -- SUBK
    ...
    { 0, 0, 1, 0, 0, 'ivABC' }, -- SETLIST (ivABC in 5.5)
    { 0, 0, 0, 0, 1, 'iABx' }, -- CLOSURE
    { 0, 1, 0, 0, 1, 'iABC' }, -- VARARG
    { 0, 0, 0, 0, 1, 'iABC' }, -- GETVARG
    { 0, 0, 0, 0, 0, 'iABx' }, -- ERRNNIL
    { 0, 0, 1, 0, 1, 'iABC' }, -- VARARGPREP
    { 0, 0, 0, 0, 0, 'iAx' }, -- EXTRAARG
}

-- offsets from lopcodes.h (same as 5.4)
local OFFSET_sC = 0x7F
local OFFSET_sBx = 0xFFFF
local OFFSET_sJ = 0xFFFFFF
...
local function decode(inst)
    ...
    -- ivABC variant: vB=6 bits at pos 16, vC=10 bits at pos 22
    local vB = (inst >> 16) & 0x3F
    local vC = (inst >> 22) & 0x3FF
    return {
        op = op,
        A = A,
        k = k,
...
local function format(d)
    ...
    elseif
        opName == 'LOADFALSE'
        or opName == 'LFALSESKIP'
        or opName == 'LOADTRUE'
        or opName == 'VARARGPREP'
    then
        return ('%d'):format(d.A)
    elseif opName == 'LOADNIL' then
        return ('%d %d'):format(d.A, d.B)
    -- ivABC: NEWTABLE, SETLIST - use vB, vC
    elseif opName == 'NEWTABLE' then
        return ('%d %d %d'):format(d.A, d.vB, d.vC)
    ...
    elseif opName == 'TFORCALL' then
        return ('%d %d'):format(d.A, d.C)
    -- 5.5: VARARG uses A B C (was A C in 5.4)
    elseif opName == 'VARARG' then
        return ('%d %d %d'):format(d.A, d.B, d.C)
    elseif opName == 'CONCAT' then
        return ('%d %d'):format(d.A, d.B)
    -- 5.5: ERRNNIL uses A Bx
    elseif opName == 'ERRNNIL' then
        return ('%d %d'):format(d.A, d.Bx)
...
local function comment(opName, d, func, pc, extra)
    ...
    elseif opName == 'CLOSURE' then
        if protos[d.Bx + 1] then
            ...
            local name = ('function_%d'):format(d.Bx)
            return ('function %s(%s)'):format(name, table.concat(params, ', '))
        end
    elseif opName == 'CALL' or opName == 'TAILCALL' then
    ...
    elseif opName == 'TFORLOOP' then
        return ('to %d'):format(pc - d.Bx)
    elseif opName == 'GETVARG' then
        return 'vararg'
    elseif opName == 'ERRNNIL' then
        local name = constants[d.Bx] and tostring(constants[d.Bx]) or ''
        return ('error if R[%d] ~= nil (%s)'):format(d.A, name)
    end
    return ''
end
    ...
    for step = 1, math.min(4, pc) do
        ...
        if prevA == d.A then
            if prevOp == 11 then
                ...
                return ('%s.%s'):format(uvname, cname)
            elseif prevOp == 13 then
                local cidx = (prev >> 24) & 0xFF
                if constants[cidx + 1] then
                    return tostring(constants[cidx + 1])
                end
            elseif prevOp == 4 then
                local bidx = (prev >> 15) & 0x1FFFF
                if constants[bidx + 1] then
                    return tostring(constants[bidx + 1])
                end
            elseif prevOp == 20 then
                local cidx = (prev >> 24) & 0xFF
                if constants[cidx + 1] then
                    return tostring(constants[cidx + 1])
                end
            elseif prevOp == 79 then
